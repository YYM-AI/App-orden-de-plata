...
if (!['localhost', '127.0.0.1'].includes(new URL(url).hostname))
  throw Error('This script only seeds the local synthetic database.');
...
try {
  ...
  if (listed.error) throw Error('Local Auth admin API unavailable.');
  for (const [key, name] of Object.entries(names)) {
    const email = `${key.toLowerCase()}@clymo.test`;
    ...
    if (!id) {
      ...
      if (r.error || !r.data.user) throw Error(`Could not create local test identity ${key}.`);
      id = r.data.user.id;
    } else if (!accounts[key]) {
      const r = await admin.auth.admin.updateUserById(id, { password });
      if (r.error) throw Error('Could not reset local-only test identity.');
    }
  ...
  for (const [label, id, name, kind, owner, helper] of households) {
    const db = createClient(url, process.env.SUPABASE_ANON_KEY!, {
      auth: { persistSession: false, autoRefreshToken: false },
    });
    const login = await db.auth.signInWithPassword(accounts[owner]);
    if (login.error) throw Error(`Local sign-in failed for ${owner}.`);
  ...
  console.log(
    'Seven local synthetic Auth identities are ready. Credentials remain only in .local/test-accounts.json (mode 0600).',
  );
} finally {
  await pg.end();
}
